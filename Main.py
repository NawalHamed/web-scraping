...
session = HTMLSession()
product_id = 1
with open('Jumia_products.csv','w',newline='') as file:#context manager

    writer = csv.writer(file)
    writer.writerow(['id','product_name','price','reviews_count','avg_rate','brand'])
    for x in range(1,50):
        ...
        for product in all_products.absolute_links:
            ...
            print(product_name, price,reviews,avg_rate,brand)
           
            writer.writerow([product_id,product_name,price,reviews, avg_rate,brand])
            product_id += 1
        print(product_id  ) 
        print('-----------------------------------------')